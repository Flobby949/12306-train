<template>
  ...
  <a-modal v-model:visible="visible" title="秒杀令牌" @ok="handleOk" ok-text="确认" cancel-text="取消">
...
const onDelete = (record) => {
  axios.delete('/business/admin/sk-token/delete/' + record.id).then((data) => {
    if (data.success) {
...
const handleOk = () => {
  axios.post('/business/admin/sk-token/save', skToken.value).then((data) => {
    if (data.success) {
...
const handleQuery = (param) => {
  ...
  loading.value = true
  axios
    .get('/business/admin/sk-token/query-list', {
      params: {
        page: param.page,
        size: param.size
      }
