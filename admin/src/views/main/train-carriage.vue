<template>
  ...
  <a-modal v-model:visible="visible" title="火车车厢" @ok="handleOk" ok-text="确认" cancel-text="取消">
    <a-form :model="trainCarriage" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
      <a-form-item label="车次编号">
        <train-select v-model="trainCarriage.trainCode"></train-select>
      </a-form-item>
...
const onDelete = (record) => {
  axios.delete('/business/admin/train-carriage/delete/' + record.id).then((data) => {
    if (data.success) {
...
const handleOk = () => {
  axios.post('/business/admin/train-carriage/save', trainCarriage.value).then((data) => {
    if (data.success) {
