  ...
  <a-modal v-model:visible="visible" title="火车车站" @ok="handleOk" ok-text="确认" cancel-text="取消">
    <a-form :model="trainStation" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="里程（公里）">
        <!-- 数字进步器 -->
        <a-input-number v-model:value="trainStation.km" :min="0" :max="10000" />
      </a-form-item>
    </a-form>
  </a-modal>
</template>
...
const onDelete = (record) => {
  axios.delete('/business/admin/train-station/delete/' + record.id).then((data) => {
    if (data.success) {
...
const handleOk = () => {
  axios.post('/business/admin/train-station/save', trainStation.value).then((data) => {
    if (data.success) {
...
watch(
  ...
  () => {
    const diff = dayjs(trainStation.value.outTime, 'HH:mm:ss').diff(
