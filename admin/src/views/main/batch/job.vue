<template>
  <div class="job">
    ...
    <a-modal title="用户" v-model:visible="modalVisible" :confirm-loading="modalLoading" @ok="handleModalOk">
...
const handleQuery = () => {
  loading.value = true
  jobs.value = []
  axios.get('/batch/admin/job/query').then((data) => {
    loading.value = false
    if (data.success) {
...
const handleModalOk = () => {
  ...
  axios.post('/batch/admin/job/' + url, job.value).then((data) => {
...
const handleResume = (record) => {
  axios.post('/batch/admin/job/reschedule', record).then((data) => {
    modalLoading.value = false
    if (data.success) {
      modalVisible.value = false
...
const handleRun = (record) => {
  axios.post('/batch/admin/job/run', record).then((data) => {
    if (data.success) {
