<template>
  ...
  <div v-else>
    ...
    <br />
    <div
      v-for="(seatObj, carriage) in train"
      :key="carriage"
      style="border: 3px solid #99ccff; margin-bottom: 30px; padding: 5px; border-radius: 4px"
    >
      <div
        style="
          display: block;
      ...
      <table>
        <tr>
          <td v-for="(sell, index) in Object.values(seatObj)[0]" :key="index" style="text-align: center">
            {{ index + 1 }}
          </td>
        </tr>
        <tr v-for="(sellList, col) in seatObj" :key="col">
          <td
            v-for="(sell, index) in sellList"
            :key="index"
...
const format = () => {
  ...
  for (let i = 0; i < list.value.length; i++) {
    ...
    // console.log(sell)
    const sell = sellDB.substr(param.value.startIndex, param.value.endIndex - param.value.startIndex)
